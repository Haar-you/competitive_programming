...
LLI rec(vector<LLI> &a, LLI l, LLI r, LLI A, LLI B){
  LLI n = lower_bound(ALL(a),r) - lower_bound(ALL(a),l);
  LLI h = (l+r)/2;

  if(n == 0){
    return A;
  }else{
    if(r-l == 1) return B*(r-l)*n;
    else return min(B*(r-l)*n, rec(a,l,h,A,B) + rec(a,h,r,A,B));
  }
}
