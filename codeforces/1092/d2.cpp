...
int solve(pair<int,int> p, stack<pair<int,int>> &st1, stack<pair<int,int>> &st2){
  ...
  if(!st1.empty() && st2.empty()){
    if(st1.top().first == p.first){
      ...
      if(p.second % 2 == 0 && st1.top().first > p.first){
  ...
  if(st1.empty() && !st2.empty()){
    if(st2.top().first == p.first){
      ...
      if(p.second % 2 == 0 && st2.top().first > p.first){
  ...
  if(p.second % 2 == 0){
    if(st1.top().first > p.first && (st1.top().first <= st2.top().first || st2.top().first < p.first)){ 
      p.first = st1.top().first;
      p.second += st1.top().second;
      st1.pop();
      return solve(p,st1,st2);
    }
    if(st2.top().first > p.first && (st2.top().first <= st1.top().first || st1.top().first < p.first)){ 
      p.first = st2.top().first;
      p.second += st2.top().second;
      st2.pop();
      return solve(p,st1,st2);
