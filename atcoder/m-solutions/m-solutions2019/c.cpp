...
int main(){
  ...
  while(cin >> N >> A >> B >> C){
    ...
    FORE(i,N,2*N-1){
      LLI a = A*mod_inv(A+B,mod)%mod;
      LLI b = B*mod_inv(A+B,mod)%mod;
      
      LLI x = com.C(i-1,N-1)*(power(a,N,mod)*power(b,i-N,mod)%mod+power(a,i-N,mod)*power(b,N,mod)%mod)%mod;
      LLI y = i*100*mod_inv(100-C,mod)%mod;
      (ans += x*y%mod) %= mod;
    }
