...
int main(){
  ...
  while(cin >> N){
    ...
    SegmentTree<int> seg(MAX+1, 0, [](int a, int b){return max(a,b);}, [](int a, int b){return max(a,b);});
    ...
    REP(i,N){
      ...
      FORE(j,1,sa){
        temp.push_back(make_pair(A[i]/j, seg.get(j,j+1) + A[i]/j));
      }

      FORE(j,1,sa){
	temp.push_back(make_pair(j, seg.get(A[i]/(j+1)+1,A[i]/j+1) + j));
      }

      for(auto &p : temp){
	seg.update(p.fst,p.snd);
      }
    }

    int ans = seg.get(0, MAX+1);
